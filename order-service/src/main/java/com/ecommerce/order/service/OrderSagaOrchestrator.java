...
import com.ecommerce.order.clients.NotificationFeignClient;
import com.ecommerce.order.clients.PaymentFeignClient;
...
public class OrderSagaOrchestrator {
	...
	public void startSaga(UUID orderId) {
		OrderSagaState oss = new OrderSagaState();
		oss.setStatus(OrderState.CREATED);
		oss.setOrderId(orderId);
		oss.setUpdatedAt(Instant.now());
		sagaStateRepository.save(oss);
		advance(oss);
	}

	@Transactional
	private void advance(OrderSagaState oss) {
		switch (oss.getStatus()) {
		case CREATED->{
			oss.setStatus(OrderState.INVENTORY_RESERVING);
			sagaStateRepository.save(oss);
			reserveInventory(oss);
		}
		case INVENTORY_RESERVED->{
			oss.setStatus(OrderState.PAYMENT_PROCESSING);
			sagaStateRepository.save(oss);
			processPayment(oss);
			
		}
		case PAYMENT_CONFIRMED->{
			oss.setStatus(OrderState.COMPLETED);
			sagaStateRepository.save(oss);
			processPayment(oss);
	...
	private void processPayment(OrderSagaState oss) {
		try {
			paymentClient.charge(oss.getOrderId());
			oss.setStatus(OrderState.PAYMENT_CONFIRMED);
			sagaStateRepository.save(oss);
			advance(oss);
		} catch (Exception e) {
			oss.setStatus(OrderState.PAYMENT_FAILED);
			oss.setLastError(e.getMessage());
			sagaStateRepository.save(oss);
			//compensating transcation
	...
	private void compensateInventory(OrderSagaState oss) {
		...
		oss.setStatus(OrderState.CANCELLED);
		sagaStateRepository.save(oss);
		notificationClient.sendFailure(oss.getOrderId());
	}

	private void reserveInventory(OrderSagaState oss) {
		try {
			inventoryClient.reserveInventory(oss.getOrderId());
			oss.setStatus(OrderState.INVENTORY_RESERVED);
			sagaStateRepository.save(oss);
			advance(oss);
		} catch (Exception e) {
